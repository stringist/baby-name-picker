...
export default function Options({ sortOrder, setSortOrder, filters, setFilters }) {
    ...
    const chooseOptions = (formData) => {
        const data = Object.fromEntries(formData)
        console.log(formData, data)
    }

    return (
        <form action={chooseOptions}>
            <fieldset className="options-wrapper">

                <legend>Options<div className="toggleOptions" onClick={toggleOptions}>X</div></legend>
                {!toggle ?
                    <div className={`optionsContent ${toggle ? 'show' : 'hide'}`}>
                        ...
                        <Button type="button" onClick={() => setSortOrder(getAlphaSortOrder())}>
                            {sortOrder === 'a-z' ? 'Sort Z-A' : 'Sort A-Z'}
                        </Button>
                        <Button type="button" onClick={() => setSortOrder('random')}>
                            Random order
                        </Button>
                    </div> : null}
            </fieldset>
